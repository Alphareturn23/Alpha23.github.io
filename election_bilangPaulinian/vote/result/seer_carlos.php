...
function _get_sitecourse_from_studentID($studentID, $table_name) 
{
	$student_data = get_where_custom("students", "student_id", $studentID);
	foreach ($student_data as $key => $row) {
		return $course = $row['site_course'];
	}
...
function count_votespercourseyear($dept_id, $year_level, $site_course)
{
	$query = "SELECT votecount.student_id, votecount.dept_id, students.year, students.site_course
	FROM votecount
	INNER JOIN students ON votecount.student_id=students.student_id
	WHERE students.year='$year_level'
	AND students.site_course='$site_course'
...
function count_deptrows($table_name, $dept_id)
{
	$conn = getConnection();
	$sql = "SELECT * FROM $table_name where dept_id='$dept_id'";
	$result = $conn->query($sql);
	$rowcount=mysqli_num_rows($result);
	return $rowcount;
}

function _get_deptid_from_candidate_id($table_name, $id) {
	$user_data = get_where_custom($table_name, "student_id", $id);
	foreach ($user_data as $key => $row) {
		return $dept_id = $row['dept_id'];
	}
